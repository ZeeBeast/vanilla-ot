...
function onSay(cid, words, param, channel)
	...
	local list = db.getResult("SELECT `r`.`id`, `r`.`player_id`, `r`.`reads`, `p`.`name` AS `player_name` FROM `server_reports` r LEFT JOIN `players` p ON `r`.`player_id` = `p`.`id`" .. tmp)
	...
	repeat
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, list:getDataInt("id") .. ", by " .. list:getDataString("player_name") .. " (" .. list:getDataInt("reads") .. " views)")
	until not list:next()
