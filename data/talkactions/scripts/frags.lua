function onSay(cid, words, param, channel)
	...
	if(getBooleanFromString(getConfigValue('advancedFragList'))) then
		local result = "Frags gained today: " .. size.day .. "/"..getConfigValue('fragsToRedSkull')
		if(size.day > 0) then
		...
		result = result .. "\nFrags gained this week: " .. (size.day + size.week) .. "/"..getConfigValue('fragsSecondToRedSkull')
		if(size.week > 0) then
		...
		result = result .. "\nFrags gained this month: " .. (size.day + size.week + size.month) .. "/"..getConfigValue('fragsThirdToRedSkull')
