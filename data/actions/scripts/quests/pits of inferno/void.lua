...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	if(getPlayerStorageValue(cid, config.storage) < 1) then
		...
		if(doPlayerAddItemEx(cid, key, false) == RETURNVALUE_NOERROR) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, getPlayerFreeCap(cid) < getItemWeight(key) and "You have found a silver key. Weighing " .. getItemWeight(key) .. " oz it is too heavy." or "You have found a silver key, but you have no room to take it.")
		end
	else
