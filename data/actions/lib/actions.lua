...
TOOLS.PICK = function(cid, item, fromPosition, itemEx, toPosition)
	...
	errors(true)
	if(isInArray(SPOTS, ground.itemid) and isInArray({354, 355, 9024, 9025}, itemEx.itemid)) then
	...
	--Naginata
	if(itemEx.uid == 5025) then
		doRemoveItem(itemEx.uid)
		doSendMagicEffect(toPosition, CONST_ME_POFF)
		return true
	end

	--Special Pickhole (Folda Dungeon)
	if(itemEx.uid == 5041) then
		doTransformItem(itemEx.uid, 392)
		doDecayItem(itemEx.uid)
		return true
	end		
	...
	if toPosition.x == 32551 and toPosition.y == 31379 and toPosition.z == 15 and 99 < getPlayerLevel(cid) then
		...
		if get1 > 0 and get2 > 0 then
			...
			doItemSetAttribute(doCreateItem(1387, 1, {x=32551, y=31376, z=15}), 'uid',  8054)
			return TRUE
		end
	end
...
TOOLS.SHOVEL = function(cid, item, fromPosition, itemEx, toPosition)
	...
	elseif(isInArray(SAND, itemEx.itemid)) then
		...
		if(isInArray(SPOTS, ground.itemid) and rand <= 20) then
			...
			doDecayItem(itemEx.uid)
		elseif((itemEx.aid == 100) and rand <= 20) then	
			doTransformItem(itemEx.uid, 489)
			doDecayItem(itemEx.uid)					
