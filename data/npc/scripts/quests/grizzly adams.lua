...
function greetCallback(cid)
	local cur, r, msg = getCreatureStorage(cid, points), getCreatureStorage(cid, rank)
...
function creatureSayCallback(cid, _type, msg)
	...
	if msgcontains(msg, 'task') then
		local cur = getCreatureStorage(cid, curTask)
		if cur < 1 or (cur ~= 14006 and getCreatureStorage(cid, cur) == tasks[data[cur][1]].amount + 1 and getCreatureStorage(cid, cur + 100) == -1) then
			...
			elseif 90 <= lvl then
				v, Topic[cid] = '{Hydras}, {sea serpents}, {behemoths}' .. (getCreatureStorage(cid, rank) == 5 and ', {serpent spawns} or {demons}' or ' or {serpent spawns}'), 3
			end
			selfSay('All right, what would you like to hunt? ' .. v .. '?', cid)
		elseif cur == 14006 and getCreatureStorage(cid, cur) == tasks['demon'].amount then
			...
			doCreatureSetStorage(cid, curTask, -1)
		elseif cur ~= 14006 and getCreatureStorage(cid, cur) == tasks[data[cur][1]].amount and getCreatureStorage(cid, cur + 100) < 1 then
			local tmp
			...
			if data[cur][6] then
				doCreatureSetStorage(cid, points, getCreatureStorage(cid, points) + data[cur][6])
			end
			if data[cur][7] then -- special task (he sends you to kill the boss)
				doCreatureSetStorage(cid, cur + 100, 1)
			end
			doCreatureSetStorage(cid, cur, getCreatureStorage(cid, cur) + 1)
			Topic[cid] = 0
		elseif getCreatureStorage(cid, cur + 100) == 2 then
			doCreatureSetStorage(cid, points, getCreatureStorage(cid, points) + 1)
			selfSay('Fantastic! If you\'re up for another hunting mission just ask me for a {task}.', cid)
			doCreatureSetStorage(cid, cur + 100, -1)
		else
			selfSay(getCreatureStorage(cid, cur + 100) < 1 and 'You are not done with your task yet. Come back to me when you are. Or do you want to abort your task to start a different one?' or string.format('Find %s\'s hideout. I hope for you that you are skilled enough to kill him. Or do you want to abort your task to start a different one?', data[cur][7]), cid)
			Topic[cid] = 4
		end
	elseif msgcontains(msg, 'trade') and getCreatureStorage(cid, rank) >= 1 then --huntsman
		local items = {
			...
			{name='slingshot', id=5907, buy=34000},
		}
		if getCreatureStorage(cid, rank) >= 3 then
		...
		if getCreatureStorage(cid, rank) >= 4 then
	...
	elseif Topic[cid] == 4 then
		...
		if msgcontains(msg, 'yes') or msgcontains(msg, 'abort') then
			...
			doCreatureSetStorage(cid, cur, -1)
			doCreatureSetStorage(cid, curTask, -1)
			if getCreatureStorage(cid, cur + 100) > 0 then
				doCreatureSetStorage(cid, cur + 100, -1)
			end
		else
			selfSay(cur == 14006 and string.format('Alright, then stop dilly dallying here and go on with your task. You have already exterminated %u demons. Come back to me when you are done.', getCreatureStorage(cid, cur)) or getCreatureStorage(cid, cur + 100) < 1 and string.format(data[getCreatureStorage(cid, curTask)][3], getCreatureStorage(cid, cur)) or 'Alright, then you should go on with your task.', cid)
		end
		Topic[cid] = 0
	...
	elseif Topic[cid] == 3 and msgcontains(msg, 'serpent spawn') then
		...
		Topic[cid] = v.storage
	elseif Topic[cid] == 3 and getCreatureStorage(cid, rank) == 5 and msgcontains(msg, 'demon') and getCreatureStorage(cid, curTask) < 1 then
		local v = tasks['demon']
		...
		doCreatureSetStorage(cid, curTask, v.storage)
		doCreatureSetStorage(cid, v.storage, 0)
		Topic[cid] = 0
	elseif Topic[cid] >= 14000 and msgcontains(msg, 'yes') then
		selfSay(data[Topic[cid]][2], cid)
		doCreatureSetStorage(cid, curTask, Topic[cid])
		doCreatureSetStorage(cid, Topic[cid], 0)
		Topic[cid] = 0
	elseif msgcontains(msg, 'special') and getCreatureStorage(cid, rank) == 5 then
		local v, k = getCreatureStorage(cid, tiquanda), getCreatureStorage(cid, demodras)
		if v < 1 then
			if getPlayerLevel(cid) >= 90 then
				doCreatureSetStorage(cid, tiquanda, 1)
		...
		elseif v == 2 then
			...
			doCreatureSetStorage(cid, tiquanda, 3)
		elseif v == 3 and k < 1 then
			if getPlayerLevel(cid) >= 100 then
				doCreatureSetStorage(cid, demodras, 1)
		...
		elseif k == 2 then
			selfSay('Great achievement, old chap! You are an outstanding hunter, no doubt about it!', cid)
			doCreatureSetStorage(cid, demodras, 3)
		end
	elseif msgcontains(msg, 'promotion') then
		local cur, r, msg, exp, item = getCreatureStorage(cid, points), getCreatureStorage(cid, rank)
		if cur >= 5 and r < 1 then
		...
		if msg then
			selfSay(msg, cid)
			doCreatureSetStorage(cid, rank, math.max(1, r + 1))
			if exp then
